...
Запускаем обучаение на определённом паиценте:
```
uv run python -m scripts.train data.subject_id=09
```
**Важно**: модель всегда обучается только на одном пациенте, список доступных пациентов(data.subject_id) пока что: `[04", "07", "08", "09", "11", "13", "14", "15", "16", "18", "22", "24", "27", "28", "29"]`
